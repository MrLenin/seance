...
- Fake lag charges the batch once, at `BATCH -` (see below), so a five-line burst costs one command's worth. Nothing was rejected and no `FAIL BATCH` was seen — but the three probes above were sent ~15 s apart, which is exactly what hides the cooldown below.
- **A delivered batch buys a cooldown, and the next batch opened inside it is dropped whole.** `ircd/m_batch.c:1795-1809` rejects a `BATCH +… draft/multiline` while `con_ml_cooldown_until` is in the future, with `FAIL BATCH MULTILINE_COOLDOWN <seconds> :…`. The cooldown is set on every _successful_ delivery (`multiline_apply_cooldown`, `:650-682`) as `(MULTILINE_COOLDOWN_BASE + total_bytes / 128) × FEAT_MULTILINE_COOLDOWN_DISCOUNT`, which at the defaults (2, 50 %) is **at least one second after even a short batch** and ~20 s after a 5 KB one; a flat two seconds also goes on `cli_since`. It is scoped to multiline batches, so everything else still works during it. The reject paths clear the batch without charging a cooldown, so only delivery counts. Two consequences for a client, both handled in `client/js/irc/multiline.ts`: a plan that needs several batches must not put them on the wire in one flush (everything after the first would be lost), and a `MULTILINE_COOLDOWN` is a "not yet", not a "no" — the batch is re-sent when the seconds it names are up rather than reported to the user.
...
## Connection throttle (IPcheck), read from source 2026-09-06

`ircd/IPcheck.c` `ip_registry_check_local` runs at accept time — before TLS, before the WebSocket upgrade — for every local connection. One registry entry per address counts `attempts`: reset to 0 when more than `IPCHECK_CLONE_PERIOD` (default 40 s, `ircd_features.c`) has passed since `last_connect`, then incremented and `last_connect` refreshed on every accept, refused ones included. Once `attempts` reaches `IPCHECK_CLONE_LIMIT` (default 4) the connection is refused ("too fast"; not for `IPCHECK_CLONE_DELAY`, 600 s, after the ircd boots). So the fourth connection in a chain of gaps of 40 s or less is refused, every further try inside the window is refused too, and only a gap longer than 40 s clears it. Over WebSocket the refusal is invisible: the socket is closed before any HTTP answer, so the browser reports 1006 exactly as for an unreachable server. A dial that never reaches the server (radio off, DNS failure) does not count. Consequences in `client/js/irc/transport.ts`: the reconnect schedule may start fast (an accepted connection that then fails is rare unless the server is rejecting us), but its capped waits stay at or above 45 s (jitter `[3/4, 1]` of the 60 s cap) so a client at the cap always gets back in, and an explicit connect restarts the schedule instead of adding to it. The dev server raises the limits (`tools/nefarious-dev/local.conf`: 100 / 1 s / 1 s) because the test suites reconnect a lot; a production network most likely runs the defaults.
...
- **Missed messages.** Clients without `draft/chathistory` get an automatic replay (`replay_start_bouncer`); with the cap the server leaves it to the client (the spec's `LATEST`-then-`BEFORE` gap fill, which is what catchup.ts does off the restored JOIN).
