...
`coffee` and `creama` implement the warm direction of the colour redesign handed off in `docs/resources/themes/` (`README.md`, `ink-amber-theme.md` and the `ink-amber-*.png` mockups); the cool direction, `cobalt-frost-theme.md` with its own mockups, is not implemented yet and would be built the same way. The handoff is colour only, and so are the themes: layout, spacing and type are untouched, and its suggestions beyond colour (a `data-theme` switch, sweeping the components for literal colours, hover/focus/mention states) were deliberately left for later — except the composer's focus ring, which is in: the bar is flush with the window, so `#form:focus-within` draws it as the top edge (accent border plus a 2px inset of `--focus-ring`) and the caret takes the accent (`caret-color`), in `coffee.css` and, through the import, `creama.css`; `style.css` gives `day` and `morning` the same in their green.
