 ...
 *
 * On touch the scrollback is `user-select: none` so the platform's long
 * press cannot race the one that opens the message action toolbar — but the
 * message whose toolbar is open is selectable again (style.css,
 * `.msg.actions-open`), so a second long press there is the platform's own:
