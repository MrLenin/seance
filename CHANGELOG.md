...
- Scrollback after a reconnect: the "show older messages" page is now asked for before the oldest message the client holds, not before the line the list happens to show first. After a reconnect that line is a local one stamped with the connect time (the replay's older messages sit below it), so the page asked for was the newest hundred, all already shown, and the same page was asked for over and over; only restarting the app got past it. A page that brings nothing new now also moves the next one past its oldest row.
