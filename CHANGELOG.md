...
- Settings → Appearance → Your own messages: greyed text (the default, as before), a highlighted row, or nothing. The grey that marks your own lines is hard to see on an iPhone with a dark theme; the setting applies to every theme (`html[data-own-messages]`, `--own-bg` for a theme that names its band).
