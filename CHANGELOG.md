...
- Replies quote their parent even when it is not loaded: the excerpt is copied when the parent was seen (so a trim cannot take it away) and fetched from the server otherwise; a parent the server has nothing for reads "(message not available)" instead of "(unknown message)".
