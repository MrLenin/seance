...
- Scrollback after a reconnect: a page of older messages asked for while the connection was already dead (a PWA back from the background, scrolled up before the transport noticed) is asked again once the channel is back, instead of leaving the view at the top with nothing more to scroll.
